...
## Quizz 
  ...
  2. Quelles sont les métadonnées disponibles ?
...
**Questions:**
  ...
  1. Comment se répartissent les articles entre les différents journaux ? 
  2. Dans quels journaux notre objet d'étude (Lyon) est-il le plus évoqué ?  
  3. Sélectionner les titres de presse qui comportent plus de 10 articles.</div>
...
<br> 
**Note:** les catégories fournies dans la source d'origine présentent une grande diversité et un certain manque de cohérence. Afin de faciliter l’analyse, un travail de regroupement a été effectué pour homogénéiser les catégories d'origine. Les choix opérés pour ces regroupements sont explicités ci-dessous, accompagnés du code utilisé pour les mettre en œuvre. 
...
Si l'on veut constituer un échantillon aléatoire, on peut appliquer la fonction **slice_sample()** du package dplyr et choisir le nombre (n =) ou la proportion du corpus (prop = ) que l'on veut conserver. La fonction **set.seed()** sert à stabiliser le résultat pour le reproduire éventuellement : 
...
Jusqu'ici, on s'en est tenu au niveau individuel du mot, ce qui présente des limites, car on l'isole de son contexte. Nous allons maintenant voir comment construire des bigrams (la même approche peut être appliquée pour des trigrammes ou plus). 
La fonction est la même que pour la tokenisation simple, il suffit de changer le nombre *n* de tokens désiré (2 pour des bigrams, 3 pour des trigrams, etc) : 
...
```
<br>
Comme pour les *unigrammes*, on s'aperçoit en inspectant les résultats qu'un nettoyage est nécessaire afin de ne conserver que les bigrammes les plus significatifs. L'opération s'effectue en trois étapes : 
  ...
  3. On reconstitue les bigrammes. 
...
On choisit d'examiner le terme clé "lyon". On propose deux approches fondées sur deux packages différents : (1) [histtext](https://bookdown.enpchina.eu/rpackage/HistTextRManual.html) (2) [quanteda](https://quanteda.io/).

## Avec HistText

1. Chargez le package **histtext** (l'installer au préalable en suivant les instructions données dans le fichier "install_histtext.R") : 
```{r message = FALSE, warning = FALSE, eval=FALSE}
...
```
<br>
2. Appliquez la fonction **search_concordance_on_df()**: 
```{r message = FALSE, warning = FALSE, eval=FALSE}
...
Le package **quanteda** requiert de transformer au préalable notre dataframe en objet "corpus" pour le rendre traitable par ses différentes fonctions : 

```{r message = FALSE, warning = FALSE, eval=FALSE}

# on transforme le dataframe en objet "corpus" traitable par quanteda 

lyon_quanteda <- corpus(lyon_corpus_filtered, text_field = "Text", docid_field = "DocId") 

# on tokenise le corpus avec la fonction tokens() de quanteda

lyon_quanteda_tokens <- tokens(lyon_quanteda)

# on applique la fonction kwic() pour voir le terme choisi dans son contexte
...
**Questions:** 
  ...
  * Quelle méthode vous semble préférable ? Justifier (La réponse peut varier en fonction de vos besoins).
...
Dans un premier temps, il est conseillé d'utiliser le package **stminsights** qui fournit une interface agréable pour faciliter l'exploration des topics obtenus et comparer les différences modèles construits. Après avoir ouvert l'application, chargez le fichier .RData qui contient votre ou vos modèles. 
```{r warning = FALSE, message = FALSE, eval = FALSE}
...
Le package **`stm`** propose une fonction appelée **`LDAvis`**, qui permet de générer une **visualisation interactive** d’un modèle de topics, inspirée des méthodes de la Latent Dirichlet Allocation (LDA). Cet outil est particulièrement utile pour **explorer et interpréter visuellement les topics** extraits du corpus. Attention : la numérotation des topics ne correspond pas à ceux obtenus avec stm! 
...
La fonction **topicCorr** permet de visualiser sous formes de graphes les chevauchement entre topics à partir des termes qu'ils partagent. Deux méthodes de calcul sont proposées, simple ou complexe (huge) (pour plus de détails sur ces mesures, voir la documentation du package [**stm**](https://www.structuraltopicmodel.com/).
...
Différentes modèles et packages sont disponibles (voir l'aide-mémoire pour une liste détaillée). Dans cette séance, nous proposons d'utiliser les modèles fournis par le package **histtext** basés sur l'ontologie [OntoNotes](https://catalog.ldc.upenn.edu/LDC2013T19).
